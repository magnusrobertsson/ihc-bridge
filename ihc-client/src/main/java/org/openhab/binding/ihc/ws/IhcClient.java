...
public class IhcClient {
	 ...
	 * @return
	 */
	public void closeConnection() throws IhcException {
	 ...
	 * @throws IhcException 
	 */
	public void openConnection() throws IhcException {
		...
		if (!loginResult.isLoginWasSuccessful()) {
			...
			if (loginResult.isLoginFailedDueToAccountInvalid()) {
				throw new IhcException("login failed because of invalid account");
			}
			
			if (loginResult.isLoginFailedDueToConnectionRestrictions()) {
				throw new IhcException("login failed because of connection restrictions");
			}

			if (loginResult.isLoginFailedDueToInsufficientUserRights()) {
				throw new IhcException("login failed because of insufficient user rights");
			}

			throw new IhcException("login failed because of unknown reason");	
		}
	 ...
	 * @throws IhcException 
	 */
	public synchronized WSProjectInfo getProjectInfo() throws IhcException {
	 ...
	 * 
	 */
	private synchronized void loadProject() throws IhcException {
		...
		if (StringUtils.isNotBlank(projectFile)) {
			...
			try {
				...
				Document doc = getProjectFileFromController();
				IhcProjectFile.parseProject(doc, dumpResourcesToFile);

			} catch (IhcException e) {
				throw new IhcException("Project file loading error", e);
			}
	...
	public Document getProjectFileFromController() throws IhcException {
		...
		try {
			...
			throw new IhcException(e);
		}

	}

	private void dumpProjectFile(byte[] decodedBytes) throws IOException {
		GZIPInputStream gzis = new GZIPInputStream(
				new ByteArrayInputStream(decodedBytes));

		InputStreamReader in = new InputStreamReader(gzis, "ISO-8859-1");
		Writer out = new BufferedWriter(new FileWriter("ihc.xml"));
		int ch = in.read();
		while (ch != -1) {
			out.write(ch);
			ch = in.read();
	...
	private List<? extends WSResourceValue> waitResourceValueNotifications(
			int timeoutInSeconds) throws IhcException, SocketTimeoutException {
	 ...
	 */
	public WSResourceValue resourceQuery(int resoureId) throws IhcException {
	 ...
	 */
	public boolean resourceUpdate(WSResourceValue value) throws IhcException {
